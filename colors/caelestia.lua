...
local function apply(p, mode)
    ...
    -- :terminal ANSI palette, so embedded shells (:terminal, Snacks.terminal,
    -- lazygit) render with the same 16 colours as foot and the desktop theme.
    -- scheme.json ships term0..term15; skip if a scheme ever omits the brights.
    local have_term = true
    for i = 0, 15 do
        if not p["term" .. i] then have_term = false break end
    end
    if have_term then
        for i = 0, 15 do vim.g["terminal_color_" .. i] = p["term" .. i] end
    end
